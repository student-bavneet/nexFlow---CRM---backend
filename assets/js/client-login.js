...
(function () {
    ...
    function setupFormValidation() {
        ...
        form.addEventListener("submit", function (e) {
            ...
            const csrfToken = document.getElementById("clientCsrfToken") ? document.getElementById("clientCsrfToken").value : "";
            const returnToInput = document.getElementById("clientReturnTo");
            const returnToVal = returnToInput ? returnToInput.value.trim() : "";
            ...
            .then(data => {
                ...
                if (data && data.success) {
                    showToast(data.message || "Sign in successful. Redirecting...", "success");
                    const redirectUrl = (data.data && data.data.redirect) ? data.data.redirect : "client-dashboard.php";
                    setTimeout(() => {
                        window.location.href = redirectUrl;
                    }, 400);
                } else {
                    const errorMsg = (data && data.message) ? data.message : "Invalid email or password.";
                    showFieldError("clientEmailGroup", errorMsg);
            ...
            .catch(err => {
                ...
                submitBtn.disabled = false;
                if (btnText) btnText.textContent = "Sign in to Client Portal";
                showFieldError("clientEmailGroup", "Unable to connect to server. Please try again.");
    ...
    // Demo Client Access Button (Prompts user without exposing credentials in code)
    function setupDemoAccessButton() {
        ...
        demoBtn.addEventListener("click", function () {
            ...
            showToast("Please enter your registered workspace email and password.", "info");
        });
    }
